...
window.Vision = (() => {
  ...
  function classify(img) {
    ...
    // **両側とも覆われると上の救済も効かない**（2026-09-07 ぴんや報告=自分ブンブン(青0.000)×相手ドンキーコング(橙0.068)で
    // 試合が丸ごと消えた）。ただし根因は救済の不足ではなく**カード窓の位置ずれ**だった:
    // 実測でカードは x663-815 / y887-960（1920換算・利用者動画と手持ち 15-18-15 で一致）なのに、旧窓は
    // 青 x670-780/y910-980・橙 x1110-1240/y900-990 ＝ 青が残る**カード上端を外し、カード下の芝を20px含んで**いた。
    // 窓をカード実寸に合わせ直す（青 x660-810・橙 x1110-1260、ともに y890-960。中心 735/1185 で左右対称）と
    // 覆われた側でも 青0.19・橙0.14〜0.18 まで戻り、素の条件（青>0.15 かつ 橙>0.10）で成立する。
    // 検証: 手持ち9動画4,834フレームの1fps走査で 陽性の最小が 青0.181/橙0.143・**負例は全て 0.000〜0.009**
    // （白ゲートだけ通る紛らわしい負例 01-44-03@1151 も 0.000 のまま）＝分離はむしろ広がった。
    if (frac(img, 90, 89, 103, 100, isWhite) > 0.25) {
      const blue = frac(img, 66, 89, 81, 96, isBlue);
      const orange = frac(img, 111, 89, 126, 96, isOrange);
